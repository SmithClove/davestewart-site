  ...
  <div class="pageShare" unselectable="on">
    ...
    <div class="pageShare__popup">
      <div ref="links" class="pageShare__links" v-show="popup">
        ...
        <a
          :href="`https://www.linkedin.com/sharing/share-offsite?url=${url}&title=${title}&summary=${description}&source=${url}`"
          target="_blank"
...
export default {
  ...
  computed: {
    ...
    description () {
      return encodeURIComponent(this.page.desc)
    },

    url () {
      return `https://davestewart.co.uk${this.page.path}`
    },

    hashtags () {
      return (this.page.frontmatter.tags || []).join(',')
    },
  },
  ...
  methods: {
    show () {
      setTimeout(() => {
        this.popup = true
        this.$nextTick(() => {
          this.$refs.links.style.marginLeft = (this.$el.offsetLeft - 1) + 'px'
        })
        const onClick = () => {
